...
class uiDroppableController {
  ...
  dragEnterHandler(event) {
    const newEvent = event.originalEvent || event;
    if (this._isAccepted(newEvent.dataTransfer.types)) {
      const index = $(newEvent.target).index();
      const counter = this._$scope._$$nrPlaceEnters[index] ? this._$scope._$$nrPlaceEnters[index]++ : this._$scope._$$nrPlaceEnters[index] = 1;
      if (counter === 1) {
  ...
  /**
   * Tries to find a child element that has the placeholder  If none was found, a new li element is created.
   */
